...
ax1.plot(df.index, df[3].rolling(window=3).mean(), linewidth=0.5)
...
ax5 = ax2.twinx()
ax5.plot(df2.index, df2[1]/1000000000,'bo', markersize=4, color = 'green')
...
ax3.plot(df.index, result.seasonal.rolling(window=3).mean(), label='Seasonal', linewidth=0.5)
ax3.set_title('c) Componente estacional')
ax3.set_xlim(start_date, end_date)

ax4.plot(df.index, result.resid.rolling(window=3).mean(), label='Residual', linewidth=0.5)
