...
print(df2)
'''
df2 = pd.read_csv('/home/emi/Documents/MAESTRIA/14-Metodología/DemandaElec/DATOS/PBI.txt', header=None, delimiter=';')
...
fig, (ax1, ax2, ax3, ax4) = plt.subplots(4, 1, figsize=(5.5, 4), sharex=True)

ax1.plot(df.index, df[3].rolling(window=3).mean(), linewidth=0.5, alpha  = 0.8)
ax1.set_title('a) Demanda')
start_date = pd.to_datetime('2007-01-01')
end_date = pd.to_datetime('2022-12-31')
ax1.set_xlim(start_date, end_date)

ax2.plot(df.index, result.trend, label='Trend', alpha  = 0.8)
ax2.set_ylim([300,420])
ax2.set_title('b) Componente de tendencia + PBI')
ax2.set_xlim(start_date, end_date)

ax5 = ax2.twinx()
ax5.plot(df2.index, df2[1]/1000000000,'bo', markersize=4, color = 'green', alpha  = 0.8)
ax5.spines['right'].set_color('green')
ax5.tick_params(axis='y', colors='green')
ax5.set_ylabel('PBI [MMU$D]', color = 'green')
ax5.set_ylim([250000,680000])

ax3.plot(df.index, result.seasonal.rolling(window=3).mean(), label='Seasonal', linewidth=0.5, alpha  = 0.8)
ax3.set_title('c) Componente estacional')
ax3.set_xlim(start_date, end_date)

ax4.plot(df.index, result.resid.rolling(window=3).mean(), label='Residual', linewidth=0.5, alpha  = 0.8)
ax4.set_title('d) Residual')
ax4.set_xlim(start_date, end_date)

fig.text(-0.004, 0.45, '[GWh]', rotation='vertical')
fig.subplots_adjust(hspace=0.1)

plt.tight_layout()
plt.savefig('decomp.png',bbox_inches="tight", dpi=600)

raw = df[3].resample("M").mean()
tend = result.trend.resample("M").mean()
st = raw - tend +raw.mean()
print(st)
